    ...
    <div className="border border-gray-300 rounded-md px-4 py-1 relative cursor-pointer hover:border-gray-400">
      ...
      {filterData && filterMenu && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 50 }}
          className="mt-2 w-48 z-40 backdrop-blur-sm max-h-44 overflow-y-scroll scrollbar-thin scrollbar-track-gray-200 scrollbar-thumb-gray-400 p-1 flex flex-col rounded-md shadow-md absolute top-8 left-0 bg-cardOverlay text-textColor"
        >
          {filterData?.map((data) => (
            <div
              key={data.name}
              className="flex items-center gap-2 px-4 py-1 hover:bg-cardHover"
